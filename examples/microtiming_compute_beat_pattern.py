...
from carat import annotations, microtiming, display
...
def compute_microtiming_pattern_for_beats(beat_annotations_file,
    ...
    ons_norm = microtiming.normalize_onsets(beat_ons, onsets)

    # 5. assigning onsets to metrical grid
    print('Assigning onsets to metrical grid ...')
    metrical_grid = microtiming.define_metrical_grid()
    ons_in_grid = microtiming.onsets_to_metrical_grid(ons_norm, metrical_grid)

    # 6. plot everything
    ax1 = plt.subplot(2, 1, 1)
    display.onsets_in_grid_plot(ons_in_grid[0], ax=ax1, hist_ons=True)
